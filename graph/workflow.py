...
------------------
  ...
  input_guard ──(blocked)──► [END]
     │ (allowed)
     ▼
  runtime_probe  ← baseline token probe + optional synthetic faults
     │
     ▼
  planner
     │
...
# Shared checkpointer so generate + HITL resume use the same thread state.
...
def build_graph(
    ...
    graph.add_edge(NODE_RUNTIME_PROBE, NODE_PLANNER)
    ...
    graph.add_conditional_edges(
        ...
        route_after_input_guard,
        {
            NODE_RUNTIME_PROBE: NODE_RUNTIME_PROBE,
            NODE_END: END,
        },
    )
    graph.add_conditional_edges(
...
def get_graph(enable_hitl: bool | None = None) -> Any:
    """Return a HITL-flag-specific compiled graph sharing one checkpointer."""
    hitl = settings.enable_hitl if enable_hitl is None else enable_hitl
    cached = _graphs.get(hitl)
    if cached is None:
        cached = build_graph(enable_hitl=hitl)
        _graphs[hitl] = cached
    return cached


def reset_graph() -> None:
    """Force rebuild of compiled graphs (e.g. after config change)."""
    _graphs.clear()
